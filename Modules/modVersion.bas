...
If DownloadFile("http://trollparty.org/LAN/version.txt", App.Path & "\Version.txt") Then
    ...
    If UBound(strFile) >= 1 Then
        If Not InStr(1, strFile(0), ".") >= 1 Or Len(strFile(0)) > 10 Then AddChat "[System] Update file is corrupt or incorrect format!": Exit Sub
        ...
        If FileExists(App.Path & "\Version.txt") Then Kill App.Path & "\Version.txt"
            If CLng(strVersion) = CLng(App.Major & App.Minor & App.Revision) Then
                AddChat "[System] Your version is up to date!"
                Exit Sub
            ElseIf CLng(strVersion) < CLng(App.Major & App.Minor & App.Revision) Then
                AddChat "[System] You have a newer version than the server!"
                Exit Sub
            ElseIf CLng(strVersion) > CLng(App.Major & App.Minor & App.Revision) Then
                AddChat "[System] There is an update available: " & App.Major & App.Minor & App.Revision & " -> " & strVersion
                If MsgBox(strChangeLog, vbYesNo, "Update Available") = vbYes Then
                        AddChat "[System] Downloading update, please wait.."
                        DoEvents
                    If DownloadFile(strDownload, App.Path & "\LanParty.New.exe") = True Then
                        AddChat "[System] Update downloaded! Updating.."
                        Sleep 1500
                        If Not isAdmin Then
                    ...
                    Else
                        AddChat "[System] There was an issue downloading the update.."
                        Exit Sub
                    End If
                Else
                    Exit Sub
    ...
    Else
        If FileExists(App.Path & "\Version.txt") Then Kill App.Path & "\Version.txt"
        AddChat "[System] Version file is corrupt or incorrect format!": Exit Sub
    End If
End If
